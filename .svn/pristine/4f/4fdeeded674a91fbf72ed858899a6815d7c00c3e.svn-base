...
<script type="text/javascript">
    ...
    function  nameCheck(id,type_name) {
        ...
        $.ajax({
            ...
            url:"customType/CustomTypeNameCheck.do",
            async:false,
            success:function (data) {
                if (data.state == "success") {
    ...
    function  codeCheck(id,type_code) {
        ...
        $.ajax({
            ...
            url:"customType/CustomTypeCodeCheck.do",
            async:false,
            success:function (data) {
                if (data.state == "success") {
                    state=true;
    ...
    function customTypeExist(id) {
        ...
        $.ajax({
            ...
            dataType:"json",
            data:{id:id},
            url:"customType/CustomTypeExist.do",
            async:false,
            success:function (data) {
                if (data.state == "success") {
                    state=true;
    ...
    function finds() {
        ...
        layui.use('table', function() {
            ...
            table.render({
                elem: '#customTypeList'
                , url: 'customType/customTypeList.do' //数据接口
                , cols: [
                    [ //表头
            ...
            table.on('tool(operation)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'del'){ //删除
                    ...
                    if(customTypeExist(id)){
                        layer.confirm('确定删除吗？', function(index){
                            ...
                            $.ajax({
                                dataType:"json",
                                data:{id:id},
                                url:"customType/deleteCustomType.do",
                                async:false,
                                success:function (data) {
                                    layer.msg("操作成功");
                    ...
                    $.ajax({
                        dataType:"json",
                        data:{id:id},
                        url:"customType/getCustomType.do",
                        async:false,
                        success:function (data) {
